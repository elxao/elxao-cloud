...
/* FIX: always do directory PROPFIND with a trailing slash */
function elxao_nc_propfind($relative){
  $rel = rtrim($relative,'/').'/';
  [$code,, $body] = elxao_nc_request('PROPFIND',$rel,['Content-Type: application/xml','Depth: 1']);
  if ($code>=400) return new WP_Error('propfind_failed','Nextcloud PROPFIND failed',['status'=>$code,'rel'=>$rel]);
  return $body;
}
...
function elxao_create_projects_from_order($order_id){
  ...
  foreach ($items as $item_id=>$item){
    ...
    update_post_meta($project_post_id,'_elxao_is_subscription_item', $is_sub_item ? 1 : 0); // for backfill

    // Provision Nextcloud folder tree
...
function elxao_api_cloud_list($request){
  ...
  $resp = elxao_nc_propfind($full); // ensures trailing slash
...
function elxao_api_cloud_download($request){
  ...
  $code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
  curl_close($ch);
  if($ok===false || $code>=400){ status_header($code?:404); exit; }
  exit;
}
...
function elxao_is_allowed_mime_value($m){ $allowed=array_map('trim',explode(',',ELXAO_CLOUD_ALLOWED_MIME)); return in_array($m,$allowed,true); }
...
function elxao_api_cloud_upload($request){
  ...
  if(elxao_is_blocked_ext($filename)) elxao_rest_error('File type not allowed (ext)',415);
...
/* ===========================================================
   ...
   - Toolbar: Back, Up, New Folder*, Rename*, Move*, Delete*, Upload (role-aware)
   - Breadcrumbs, grid/list toggle, keyboard support (Enter, Backspace)
   - Clients: browse everywhere, upload only in /Uploads
   =========================================================== */
...
/* Inline JS/CSS (Explorer) */
add_action('wp_enqueue_scripts', function(){
  if (wp_script_is('elxao-cloud-explorer-js','registered')) return;
  wp_register_script('elxao-cloud-explorer-js','',[], '1.25.1', true);
...
(function(){
  ...
  function setRoleActions(root){
    const role = root.getAttribute('data-role')||'client';
    const isClient = role==='client';
    ['.ex-newfolder','.ex-rename','.ex-move','.ex-delete'].forEach(sel=>{
      root.querySelector(sel).disabled = isClient;
    });
  }
  ...
  function renderBreadcrumb(root, path){
    const el=root.querySelector('.ex-breadcrumb'); if(!el) return;
    const parts=segs(path); let acc=''; const out=['<span class="ex-crumb" data-p="">Project</span>'];
    parts.forEach((p,i)=>{acc = i===0?p:acc+'/'+p; out.push('<span class="ex-sep">›</span><span class="ex-crumb'+(i===parts.length-1?' active':'')+'" data-p="'+acc+'">'+p+'</span>');});
    el.innerHTML=out.join('');
    el.querySelectorAll('.ex-crumb').forEach(c=>{c.onclick=()=>{S.path=c.getAttribute('data-p')||''; load(root);};});
  }
  ...
  function renderItems(root, items){
    ...
    const grid=root.querySelector('.ex-gridview'); const list=root.querySelector('.ex-listview'); const tbody=list.querySelector('tbody');
    grid.innerHTML=''; tbody.innerHTML='';

    items.forEach(it=>{
      const card=document.createElement('button'); card.type='button'; card.className='ex-card '+(it.type==='dir'?'dir':'file');
      card.setAttribute('data-name',it.name);
      card.innerHTML='<div class="ex-ico"></div><div class="ex-name" title="'+it.name+'">'+it.name+'</div>';
      card.onclick=()=> itemOpen(root,it);
      card.oncontextmenu=(e)=>{ e.preventDefault(); select(root,it.name,card); };
      grid.appendChild(card);

      const tr=document.createElement('tr'); tr.setAttribute('data-name',it.name);
      tr.innerHTML='<td>'+it.name+'</td><td>'+it.type+'</td><td>'+fmtSize(it.size)+'</td><td>'+ (it.mtime||'') +'</td>';
      tr.ondblclick=()=> itemOpen(root,it);
      tr.onclick=()=> select(root,it.name,tr);
  ...
  function select(root,name,el){
    root.querySelectorAll('.ex-card.selected, .ex-listview tr.selected').forEach(x=>x.classList.remove('selected'));
    if(el){ el.classList.add('selected'); S.selected=name; } else { S.selected=null; }
  }

  function itemOpen(root,it){
    if(it.type==='dir'){
      S.path = joinPath(S.path,it.name);
      load(root);
    } else {
      const url=api(ELXAO_CLOUD.restBase,'cloud/download',{project_id:ELXAO_CLOUD.projectId,path:joinPath(S.path,it.name)});
      window.open(url,'_blank');
    }
  }

  function updateUploadAvailability(root, path){
    const role = root.getAttribute('data-role')||'client';
    const btn = root.querySelector('.ex-upload');
    if(role!=='client'){ btn.disabled=false; return; }
    const sub=ELXAO_CLOUD.uploadsSub;
    const allowed = (path===sub) || (path && path.startsWith(sub+'/'));
  ...
  function attachToolbar(root){
    ...
    const nf=root.querySelector('.ex-newfolder');
    const rn=root.querySelector('.ex-rename');
    const mv=root.querySelector('.ex-move');
    const del=root.querySelector('.ex-delete');
    const upbtn=root.querySelector('.ex-upload');
    const file=root.querySelector('.ex-file');
    const toggle=root.querySelector('.ex-view');

    back.onclick=()=>{ S.history.pop(); const prev=S.history.pop(); if(prev===undefined){ S.path=''; } else { S.path=prev; } load(root); };
    up.onclick=()=>{ S.path = upPath(S.path); load(root); };

    nf.onclick=()=>{
      const name=prompt('New folder name:'); if(!name) return;
      fetch(api(ELXAO_CLOUD.restBase,'cloud/mkdir',{project_id:ELXAO_CLOUD.projectId,path:S.path}),{method:'POST',headers:Object.assign({'Content-Type':'application/json'},restHeaders()),body:JSON.stringify({name})})
        .then(r=>r.json()).then(j=>{ if(!j.ok) throw new Error(j.message||'MKDIR failed'); load(root,false); })
        .catch(e=>alert(e.message||'MKDIR error'));
    };

    rn.onclick=()=>{
      if(!S.selected) return alert('Select an item');
      const newname=prompt('Rename to:', S.selected); if(!newname||newname===S.selected) return;
      fetch(api(ELXAO_CLOUD.restBase,'cloud/rename',{project_id:ELXAO_CLOUD.projectId,path:joinPath(S.path,S.selected)}),{method:'POST',headers:Object.assign({'Content-Type':'application/json'},restHeaders()),body:JSON.stringify({new_name:newname})})
        .then(r=>r.json()).then(j=>{ if(!j.ok) throw new Error(j.message||'Rename failed'); load(root,false); })
        .catch(e=>alert(e.message||'Rename error'));
    };

    mv.onclick=()=>{
      if(!S.selected) return alert('Select an item');
      const to=prompt('Move to (path relative to project root):', upPath(joinPath(S.path,S.selected))); if(to===null) return;
      fetch(api(ELXAO_CLOUD.restBase,'cloud/move',{project_id:ELXAO_CLOUD.projectId,path:joinPath(S.path,S.selected)}),{method:'POST',headers:Object.assign({'Content-Type':'application/json'},restHeaders()),body:JSON.stringify({to})})
        .then(r=>r.json()).then(j=>{ if(!j.ok) throw new Error(j.message||'Move failed'); load(root,false); })
        .catch(e=>alert(e.message||'Move error'));
    };

    del.onclick=()=>{
      if(!S.selected) return alert('Select an item');
      if(!confirm('Delete "'+S.selected+'"?')) return;
      fetch(api(ELXAO_CLOUD.restBase,'cloud/delete',{project_id:ELXAO_CLOUD.projectId,path:joinPath(S.path,S.selected)}),{method:'POST',headers:restHeaders()})
        .then(r=>r.json()).then(j=>{ if(!j.ok) throw new Error(j.message||'Delete failed'); load(root,false); })
        .catch(e=>alert(e.message||'Delete error'));
    };
    ...
    file.onchange=()=>{
      ...
      const url=api(ELXAO_CLOUD.restBase,'cloud/upload',{project_id:ELXAO_CLOUD.projectId,path:S.path});
      fetch(url,{method:'POST',headers:Object.assign({'Content-Type':'application/octet-stream','X-File-Name':f.name,'X-File-Type':(f.type||'')},restHeaders()),body:f})
        .then(r=>r.json()).then(j=>{ if(!j||!j.ok) throw new Error(j&&j.message||'Upload failed'); })
        .catch(err=>alert(err.message||'Upload error'))
        .finally(()=>{ upbtn.disabled=false; upbtn.textContent='Upload'; file.value=''; load(root,false); });
    };
    ...
    root.addEventListener('drop',e=>{
      ...
      fetch(url,{method:'POST',headers:Object.assign({'Content-Type':'application/octet-stream','X-File-Name':f.name,'X-File-Type':(f.type||'')},restHeaders()),body:f})
        .then(r=>r.json()).then(j=>{ if(!j||!j.ok) throw new Error(j&&j.message||'Upload failed'); })
        .catch(err=>alert(err.message||'Upload error'))
        .finally(()=> load(root,false));
    });

    toggle.onclick=()=>{
      S.grid=!S.grid;
      const grid=root.querySelector('.ex-gridview'); const list=root.querySelector('.ex-listview');
      if(S.grid){ grid.style.display='grid'; list.style.display='none'; toggle.classList.add('ex-grid'); }
      else { grid.style.display='none'; list.style.display='table'; toggle.classList.remove('ex-grid'); }
    };

    root.addEventListener('keydown', (e)=>{
      if(e.key==='Backspace'){ e.preventDefault(); S.path=upPath(S.path); load(root); }
      if(e.key==='Enter' && S.selected){
        const el = root.querySelector('.ex-card.selected') || root.querySelector('.ex-listview tr.selected');
        if(el){
          const name=el.getAttribute('data-name'); if(!name) return;
          const itType = el.classList.contains('dir') ? 'dir' : (el.querySelector('td:nth-child(2)')?.textContent||'file');
          itemOpen(root,{name, type: itType});
        }
      }
    });
  }
...
JS);
  wp_register_style('elxao-cloud-explorer-css', false, [], '1.25.1');
