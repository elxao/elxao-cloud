...
function elxao_render_cloud_shortcode($atts)
{
    ...
    ?>
    <div class="elxao-explorer-wrapper" data-project-id="<?php echo esc_attr($pid); ?>" data-role="<?php echo esc_attr($role); ?>">
...
(function(){
  // Helper to get instance-specific state
  function getState(root){
    if(!root.__elxaoState){
      root.__elxaoState = { path:'', history:[], grid:true, selected:null };
    }
    return root.__elxaoState;
  }
  // Helper to get project ID from wrapper
  function getProjectId(root){
    const wrapper = root.closest('.elxao-explorer-wrapper');
    return wrapper ? wrapper.getAttribute('data-project-id') : null;
  }
  ...
  function load(root, pushHist = true){
    ...
    const url = api(ELXAO_CLOUD.restBase, 'cloud/list', {project_id: projectId, path: S.path});
  ...
  function renderBreadcrumb(root, path){ const S = getState(root); const el = root.querySelector('.ex-breadcrumb'); if(!el) return; const parts = segs(path); let acc = ''; const out = ['<span class="ex-crumb" data-p="">Project</span>']; parts.forEach((p,i) => { acc = i === 0 ? p : acc + '/' + p; out.push('<span class="ex-sep">›</span><span class="ex-crumb' + (i === parts.length - 1 ? ' active' : '') + '" data-p="' + acc + '">' + p + '</span>'); }); el.innerHTML = out.join(''); el.querySelectorAll('.ex-crumb').forEach(c => { c.onclick = () => { S.path = c.getAttribute('data-p') || ''; load(root); }; }); }
  function renderItems(root, items){ const S = getState(root); S.selected = null; items.sort((a,b) => { if(a.type !== b.type) return a.type === 'dir' ? -1 : 1; return a.name.localeCompare(b.name); }); const grid = root.querySelector('.ex-gridview'); const list = root.querySelector('.ex-listview'); const tbody = list.querySelector('tbody'); grid.innerHTML = ''; tbody.innerHTML = ''; items.forEach(it => {
      const card = document.createElement('button'); card.type = 'button'; card.className = 'ex-card ' + (it.type === 'dir' ? 'dir' : 'file'); card.setAttribute('data-name', it.name);
      const cardIco = document.createElement('div'); cardIco.className = 'ex-ico'; card.appendChild(cardIco);
      const cardName = document.createElement('div'); cardName.className = 'ex-name'; cardName.title = it.name || ''; cardName.textContent = it.name || ''; card.appendChild(cardName);
      card.onclick = () => itemOpen(root,it); card.oncontextmenu = (e) => { e.preventDefault(); select(root, it.name, card); }; grid.appendChild(card);
      ...
      const tdType = document.createElement('td'); tdType.textContent = it.type || ''; tr.appendChild(tdType);
      const tdSize = document.createElement('td'); tdSize.textContent = fmtSize(it.size); tr.appendChild(tdSize);
      const tdMtime = document.createElement('td'); tdMtime.textContent = it.mtime || ''; tr.appendChild(tdMtime);
      tr.ondblclick = () => itemOpen(root,it); tr.onclick = () => select(root, it.name, tr); tbody.appendChild(tr);
    }); if(S.grid){ grid.style.display = 'grid'; list.style.display = 'none'; } else { grid.style.display = 'none'; list.style.display = 'table'; } }
  function select(root,name,el){ const S = getState(root); root.querySelectorAll('.ex-card.selected, .ex-listview tr.selected').forEach(x => x.classList.remove('selected')); if(el){ el.classList.add('selected'); S.selected = name; } else { S.selected = null; } }
  function itemOpen(root,it){ const S = getState(root); const projectId = getProjectId(root); if(!projectId){ alert('Missing project ID'); return; } if(it.type === 'dir'){ S.path = joinPath(S.path, it.name); load(root); } else { const url = api(ELXAO_CLOUD.restBase, 'cloud/download', {project_id: projectId, path: joinPath(S.path, it.name)}); window.open(url, '_blank'); } }
